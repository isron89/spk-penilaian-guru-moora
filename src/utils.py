...
def moora(tahun):
    ...
    recommendation = [
        'Naik gaji', 'Baik, pertahankan', 'Cukup, tingkatkan', 'Kurang, perbaiki', '-','-'
    ]
    ...
    return df.iloc[:,:4].join(pd.DataFrame(mcdma.moora_method(nilai.iloc[:,4:].values,w,c,graph=False)[:,1],columns=["skor"])).sort_values(by='skor',ascending=False).set_index('uid').join(pd.DataFrame(recommendation,columns=["rekomendasi"])).join(profil.set_index('uid')).reset_index()[['nip','nama','gol','mata_pelajaran','skor','rekomendasi']].reset_index().rename(columns={
        'index':'Peringkat','nip':'NIP','nama':'Nama','gol':'Golongan','mata_pelajaran':'Mata Pelajaran','skor':'Skor','rekomendasi':'Rekomendasi', \
    }), df.rename(columns={n:kriteria[kriteria['indikator_id'].str[0]==n]['kompetensi'].unique()[0] for n in kriteria['indikator_id'].str[0].unique()})
